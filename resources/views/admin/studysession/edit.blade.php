...
@section('content')
    <div>
        ...
        <div class="bg-white p-6 rounded-lg shadow-md mb-8">
            ...
            <form action="{{ route('studysession.update', $studysession) }}" method="POST" class="mb-6">
                @csrf
                @method('PUT')
                <div class="flex space-x-4">
                    <div class="flex-1">
                        ...
                        <input type="text" name="name" id="name" placeholder="Example: 2021/2022"
                            value="{{ $studysession->name }}"
                            class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md">
                    </div>
                    <div class="flex-1">
                        <label for="start_date" class="block text-sm font-medium text-gray-700 mb-2">Start Date</label>
                        <input type="date" name="start_date" id="start_date" value="{{ $studysession->start_date }}"
                            class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md">

                    </div>
                    <div class="flex-1">
                        <label for="end_date" class="block text-sm font-medium text-gray-700 mb-2">End Date</label>
                        <input type="date" name="end_date" id="end_date" value="{{ $studysession->end_date }}"
                            class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md">
                ...
                <button type="submit"
                    class="mt-4 bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600 transition duration-200">Update
                    Session
                </button>
            </form>
        </div>
